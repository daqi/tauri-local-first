  ...
  const runAction = async (app: AppDescriptor, action: AppAction) => {
    ...
    if (argSpecs.length > 0) {
      ...
      for (const spec of argSpecs) {
        const v = window.prompt(`请输入参数 ${spec.name}${spec.required ? ' (必填)' : ''}`) ?? '';
        if (v || spec.required) {
            ...
            <Flex direction="column" gap="3">
              {apps.map((app) => (
                <Box key={app.id}>
                  ...
                  {app.description ? <Text color="gray">{app.description}</Text> : null}
                  <Flex gap="2" wrap="wrap" mt="2">
                    {(app.actions || []).map((act) => (
                      <Button key={act.name} variant="soft" onClick={() => runAction(app, act)}>
                        {act.title || act.name}
                      </Button>
                    ))}
                  </Flex>
