...
import { ActionList, ExecutionStatus, HistoryList, ExecuteActionResult } from '@suite/ui';
    ...
    <div className="flex flex-col gap-4 p-4 max-w-3xl mx-auto">
      <div className="flex gap-2 items-start">
        ...
        <div className="flex flex-col gap-2 w-32">
          <button className="px-2 py-1 text-sm border rounded" onClick={() => parse(false)}>
            Parse
          </button>
          <button
          ...
          <button
            className="px-2 py-1 text-sm border rounded bg-green-600 text-white disabled:opacity-50"
            onClick={() => run(false)}
            disabled={executing}
          >
            Execute
      ...
      <div className="space-y-2">
        ...
        <ActionList
          actions={actions.map((a: ExecuteActionResult) => ({
            intentId: a.intentId,
            status: a.status,
            reason: a.reason || undefined,
            durationMs: a.durationMs || undefined,
            predictedEffects: a.predictedEffects,
          }))}
        />
      </div>
      <div className="space-y-2">
