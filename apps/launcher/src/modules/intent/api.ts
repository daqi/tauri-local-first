...
export async function parseIntent(params: ParseIntentParams): Promise<ParsedPlanSummary> {
  const resp = await invoke<any>('parse_intent', {
    req: { input: params.input, explain: !!params.explain },
  });
...
export async function dryRun(params: ExecuteParams): Promise<ExecuteResponse> {
  const resp = await invoke<any>('dry_run', {
    req: { input: params.input, planId: params.planId, dryRun: true },
  });
  return resp as ExecuteResponse;
}
export async function executePlan(params: ExecuteParams): Promise<ExecuteResponse> {
  const resp = await invoke<any>('execute_plan', {
    req: { input: params.input, planId: params.planId, dryRun: false, timeoutMs: params.timeoutMs },
  });
  return resp as ExecuteResponse;
}
