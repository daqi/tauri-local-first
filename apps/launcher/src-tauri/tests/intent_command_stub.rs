...
async fn parse_intent_success() {
    ...
    assert!(
        resp.get("cacheHit").and_then(|v| v.as_bool()).is_some(),
        "cacheHit bool present"
    );
...
async fn dry_run_success_with_input() {
    ...
    assert_eq!(
        resp.get("overallStatus").and_then(|v| v.as_str()),
        Some("success")
    );
    assert!(
        resp.get("cacheHit").and_then(|v| v.as_bool()).is_some(),
...
async fn dry_run_success_with_plan_id() {
    ...
    let parse_resp = intent::parse_intent(intent::IntentParseRequest {
        input: "hosts:switch(dev)".into(),
        explain: false,
    })
    .await
...
async fn execute_plan_success_with_input() {
    ...
    assert_eq!(
        actions[0].get("status").and_then(|v| v.as_str()),
        Some("success")
    );
}
