use intent_core::{RuleBasedParser, ParseOptions, build_plan, simulate_plan, scan};
...
fn dry_run_pipeline_success() {
    ...
    std::fs::write(app_dir.join("tlfsuite.json"), r#"{"app_id":"hosts","actions":[{"name":"switch"}]}"#).unwrap();
