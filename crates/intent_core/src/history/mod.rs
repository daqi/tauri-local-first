...
impl InMemoryHistoryStore {
    pub fn new(retention_days: u64) -> Self {
        Self { data: BTreeMap::new(), retention_ms: retention_days * 24 * 60 * 60 * 1000 }
    }

    fn adjusted_key(&self, ts: u64) -> u64 {
        // ensure uniqueness by bumping timestamp if collision
        let mut k = ts;
        while self.data.contains_key(&k) { k += 1; }
        k
    }
}
...
impl HistoryStore for InMemoryHistoryStore {
    ...
    fn purge_older_than(&mut self, cutoff_ts: u64) -> usize {
        let keys: Vec<u64> = self.data.keys().cloned().filter(|k| *k < cutoff_ts).collect();
        let count = keys.len();
        for k in keys { self.data.remove(&k); }
        count
    }
}
