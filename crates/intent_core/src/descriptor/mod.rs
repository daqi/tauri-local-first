...
pub fn scan<P: AsRef<Path>>(roots: &[P]) -> ScanResult {
    ...
    for root in roots {
        ...
        let entries = match fs::read_dir(root_path) {
            Ok(e) => e,
            Err(e) => {
                issues.push(DescriptorLoadIssue {
                    app_id: None,
                    level: "PARSE_ERROR".into(),
            ...
            if path.is_dir() {
                ...
                if file.exists() {
                    match load_descriptor(&file) {
                        ...
                        Err(e) => issues.push(DescriptorLoadIssue {
                            app_id: None,
                            level: "PARSE_ERROR".into(),
                            message: e.to_string(),
...
fn load_descriptor(
    path: &Path,
) -> Result<(Option<ApplicationDescriptor>, Vec<DescriptorLoadIssue>), anyhow::Error> {
    ...
    let app_id = match value.get("app_id").and_then(|v| v.as_str()) {
        ...
        _ => {
            issues.push(DescriptorLoadIssue {
                app_id: None,
                level: "SCHEMA_ERROR".into(),
                message: "missing app_id".into(),
                path: path.display().to_string(),
            });
    ...
    for a in actions_val {
        let name = match a.get("name").and_then(|v| v.as_str()) {
            ...
            None => {
                issues.push(DescriptorLoadIssue {
                    ...
                    level: "SCHEMA_ERROR".into(),
                    message: "action missing name".into(),
                    path: path.display().to_string(),
                });
                continue;
            }
        ...
        if !names.insert(name.clone()) {
            issues.push(DescriptorLoadIssue {
                app_id: Some(app_id.clone()),
                level: "SEMANTIC_ERROR".into(),
                message: format!("duplicate action name: {name}"),
                path: path.display().to_string(),
            });
        }
        let requires_elevation = a
...
mod tests {
    ...
    fn semantic_duplicate_action() {
        ...
        write_file(
            &file,
            r#"{"app_id":"x","actions":[{"name":"a"},{"name":"a"}]}"#,
        );
        let res = scan(&[dir.path()]);
    ...
    fn ok_descriptor() {
        ...
        write_file(
            &file,
            r#"{"app_id":"ok","actions":[{"name":"run","requires_elevation":true}]}"#,
        );
        let res = scan(&[dir.path()]);
        assert_eq!(res.descriptors.len(), 1);
