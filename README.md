...
## 应用清单与进度
- 剪贴板管家（历史/去重/模糊搜索/全局快捷键/托盘）【building】
  - [ ] 托盘与全局快捷键
  - [ ] 文本/图片记录与本地持久化（SQLite）
  - [ ] 搜索、去重、粘贴增强
  - [ ] 设置页与数据导出
- 本地笔记（Markdown + 双链/标签 + FTS5 搜索）【planned】
  - [ ] 笔记库与附件目录
  - [ ] Markdown 编辑与预览
  - [ ] FTS5 搜索与反向链接
  - [ ] 导入/导出
- 图片批量压缩/转换（PNG/JPEG/WebP）【planned】
  - [ ] 拖放批处理
  - [ ] 并发任务队列
  - [ ] 质量预设与预览
  - [ ] 元数据处理（EXIF 可选保留）
- 截图与离线 OCR（Tesseract）【planned】
  - [ ] 区域截图
  - [ ] 本地 OCR
  - [ ] 一键复制/保存
- PDF 工具箱（合并/拆分/提取/压缩）【planned】
- JSON/CSV 工具箱（校验/格式化/转换/大文件）【planned】
- 日志查看器（超大文件 tail/搜索/书签/虚拟列表）【planned】
- 文件去重/清理（分块哈希/感知哈希/相似图片）【planned】
- 轻量 API 客户端（环境/脚本/历史/本地加密）【planned】
- 番茄钟 + 时间追踪（托盘/统计/导出）【planned】
